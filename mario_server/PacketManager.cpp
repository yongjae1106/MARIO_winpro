...
// ==============================================================
int PacketManager::SerializePacket(char* buffer, const PacketInfo_ServerToClient& pkt)
{
    int offset = 0;
    ...
    int eventCount = (int)pkt.events.size();
    memcpy(buffer + offset, &eventCount, sizeof(int)); offset += sizeof(int);

    for (auto& e : pkt.events)
    {
        int val = static_cast<int>(e);
        memcpy(buffer + offset, &val, sizeof(int)); offset += sizeof(int);
    }
