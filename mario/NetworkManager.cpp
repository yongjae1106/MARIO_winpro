...
void NetworkManager::Stop() {
    if (isRunning) 
    {
        ...
        {
            // NetworkLoop()가 종료될 때까지 기다린 후 네트워크 스레드 블로킹
            networkThread.join();
        }
...
bool NetworkManager::Connect(const std::string& ipAddress, int port) 
{
    ...
    // 5초간 연결완료를 기다리는 반블로킹 형태의 connecting
    int result = connect(clientSocket, (SOCKADDR*)&serverAddr, sizeof(serverAddr));
    if (result == SOCKET_ERROR)
    {
        // connect()가 바로 실패했을 때, WSAEWOULDBLOCK은 "아직 진행 중"을 의미함
        if (WSAGetLastError() == WSAEWOULDBLOCK)
        {
            // 연결이 아직 완료되지 않았으므로, select()로 연결 완료 여부를 기다림
            fd_set writeSet;                        // 쓰기 가능한 소켓을 감시할 fd_set
            FD_ZERO(&writeSet);                     // fd_set 초기화
            FD_SET(clientSocket, &writeSet);        // 감시 대상 소켓 등록

            timeval timeout;
            timeout.tv_sec = 5;                     // 최대 5초 동안 기다림
            timeout.tv_usec = 0;

            // select() 호출: 쓰기 가능한 소켓을 기다림
            ...
            {
                // select()가 성공적으로 반환됨 → 소켓이 쓰기 가능 상태일 수 있음
                // 하지만 실제로 연결이 완료되었는지 getsockopt()로 다시 확인해야 함
                int optval;
